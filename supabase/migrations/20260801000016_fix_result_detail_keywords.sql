-- ============================================================================
-- Fix fn_result_detail runtime error (400 on every call).
-- ----------------------------------------------------------------------------
-- The essay_keywords CASE mixed text[] and jsonb branches, which Postgres
-- cannot unify ("CASE types jsonb and text[] cannot be matched"). The function
-- created successfully, so `db push` reported no error — but every teacher
-- review AND every student result page failed at runtime with 400.
-- Fix: convert the array to jsonb with to_jsonb().
-- ============================================================================
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
      ...
      'essay_keywords', case when v_role = 'teacher' then to_jsonb(coalesce(q.essay_keywords, '{}')) else '[]'::jsonb end,
